...
import { APARTMENTS_UPDATED_EVENT } from "../utils/apartments";
import { BOOKINGS_UPDATED_EVENT } from "../utils/bookings";
import { getAllApartments } from "../utils/apartments";
import { formatBookingDate, getBookings, isBookingApproved } from "../utils/bookings";
...
function statusLabel(status: AdminApartment["status"] | Booking["status"], isArabic: boolean) {
  const values = isArabic
    ? { pending: "معلق", approved: "مقبول", rejected: "مرفوض", confirmed: "مؤكد" }
    : { pending: "Pending", approved: "Approved", rejected: "Rejected", confirmed: "Confirmed" };
    ...
    <main className="dashboard admin-dashboard" key={version}>
      <aside>
        <div className="current-user">
          ...
          <span><b>{currentUser.name}</b><small>ADMIN</small></span>
        </div>
        {menu.map(([key, label]) => <button className={view === key ? "active" : ""} key={key} onClick={() => setView(key)}>{label}</button>)}
        <Link to="/profile">{isArabic ? "الملف الشخصي" : "Profile"}</Link>
        <Link to="/admin/favorites">{text.favorites}</Link>
        <button className="logout-link" onClick={() => { logout(); window.location.assign("/"); }}>{isArabic ? "تسجيل الخروج" : "Log out"}</button>
      </aside>
      <section>
        <small>{isArabic ? "لوحة تحكم الأدمن" : "ADMIN OVERVIEW"}</small>
        <h1>{view === "overview" ? (isArabic ? "ملخص المنصة" : "Platform pulse") : menu.find(([key]) => key === view)?.[1]}</h1>
        ...
        {view === "users" && <UserList users={users} text={text} currentUserId={currentUser.id} onDetails={setSelectedUser} onDelete={removeUser} />}
        {(view === "bookings" || view === "requests") && <BookingList bookings={view === "requests" ? pendingBookings : bookings} text={text} isArabic={isArabic} />}
      </section>
...
function ApartmentList({ apartments, text, isArabic, onDetails, onApprove, onReject, onDelete }: { apartments: AdminApartment[]; text: AdminLabels; isArabic: boolean; onDetails: (apartment: AdminApartment) => void; onApprove?: (id: number) => void; onReject?: (id: number) => void; onDelete: (id: number) => void }) {
  ...
  return <div className="panel admin-list">{apartments.map((apartment) => <div className="admin-record" key={apartment.id}><img src={apartment.image} alt={apartment.area} /><span><b>{apartment.area}, {apartment.city}</b><small>{apartment.publisherName} · {apartment.publisherRole} · {apartment.price.toLocaleString()} EGP</small><em>{statusLabel(apartment.status, isArabic)}</em></span><div className="admin-actions"><button className="btn" onClick={() => onDetails(apartment)}>{text.details}</button>{onApprove && <button className="btn" onClick={() => onApprove(apartment.id)}>{text.approve}</button>}{onReject && <button className="btn" onClick={() => onReject(apartment.id)}>{text.reject}</button>}<button className="btn" onClick={() => onDelete(apartment.id)}>{text.delete}</button></div></div>)}</div>;
}

function UserList({ users, text, currentUserId, onDetails, onDelete }: { users: ReturnType<typeof getAdminUsers>; text: AdminLabels; currentUserId: string; onDetails: (user: User) => void; onDelete: (id: string) => void }) {
  return <div className="panel admin-list">{users.map((user) => <div className="admin-record" key={user.id}><span><b>{user.name}</b><small>{user.email} · {user.phone || text.noData}</small><em>{user.role}</em></span><div className="admin-actions"><button className="btn" onClick={() => onDetails(user)}>{text.details}</button>{user.role !== "ADMIN" && user.id !== currentUserId && <button className="btn" onClick={() => onDelete(user.id)}>{text.delete}</button>}</div></div>)}</div>;
}
  ...
  return <div className="panel admin-list">{bookings.map((booking) => {
    ...
    return <div className="admin-record" key={booking.bookingId}>
      <span>
        ...
        <small>{text.type}: {booking.bookingType} · {text.date}: {formatBookingDate(booking.bookingDate, isArabic ? "ar" : "en") || text.noData}</small>
...
function ApartmentDetailsModal({ apartment, text, isArabic, onClose }: { apartment: AdminApartment; text: AdminLabels; isArabic: boolean; onClose: () => void }) {
  ...
  return <div className="admin-modal" role="dialog" aria-modal="true">
    <div className="panel admin-property-modal">
      ...
      <div className="admin-property-facts"><p><b>{statusText.price}:</b> {apartment.price.toLocaleString()} EGP</p><p><b>{statusText.city}:</b> {apartment.city}</p><p><b>{statusText.area}:</b> {apartment.area}</p><p><b>{statusText.district}:</b> {apartment.district}</p><p><b>{statusText.address}:</b> {apartment.address}</p><p><b>{statusText.building}:</b> {apartment.buildingNumber}</p><p><b>{statusText.floor}:</b> {apartment.floorNumber}</p><p><b>{statusText.allowedGender}:</b> {apartment.allowedGender}</p><p><b>{statusText.publisher}:</b> {apartment.publisherName} · {apartment.publisherPhone || text.noData}</p><p><b>{statusText.amenities}:</b> {apartment.amenities.join(", ") || text.noData}</p></div>
      <div className="admin-room-list">{rooms.map((room) => <div className="admin-room" key={room.number}><div className="admin-room-heading"><h3>{statusText.room} {room.number}</h3><em>{statusLabelForRoom(getRoomStatus(room.beds))}</em></div>{room.beds.map((bed) => <div className="admin-bed" key={bed.number}><span>{statusText.bed} {bed.number}</span><em>{bed.booking ? statusText.reserved : statusText.available}</em>{bed.booking && <small>{statusText.booking}: {bed.booking.studentName || text.noData} · {statusText.phone}: {bed.booking.studentPhone || text.noData} · {statusText.college}: {bed.booking.collegeOrWork || text.noData} · {statusText.type}: {bed.booking.bookingType} · {statusText.status}: {statusLabel(bed.booking.status, isArabic)}</small>}</div>)}</div>)}</div>
    </div>
  </div>;
}
